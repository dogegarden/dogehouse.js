...
export default (app: Client) => {
  app.socket.addEventListener("message", (e, _arrivedId) => {
    ...
    if (msg.op === OP_CODE.HAND_RAISED) {
      app.emit(EVENT.HAND_RAISED, app.users.get(msg.d.userId));
    }

    if (msg.op === OP_CODE.JOIN_ROOM_DONE) {
      app.emit(EVENT.BOT_JOINED_ROOM, msg.d);
    }

    if (botSpeakerCodes.includes(msg.op)) {
      app.emit(EVENT.WEBRTC_VOIC_OPTS_REVIEVED, msg.d);
    }
    ...
    if (msg.op === OP_CODE.NEW_CHAT_MESSAGE) {
      ...
      if (app.incommingChatMessageListeners.has(text)) {
        app.incommingChatMessageListeners
          .get(text)
          .forEach((fn) => fn(controller));
