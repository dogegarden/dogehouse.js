...
import { ERROR, EVENT, OP_CODE, TIMEOUT } from "../util/constraints";
...
class Rooms {
  ...
  setRoomData(data) {
    if (!data?.id) return;
    const controller = this._roomControllerCache.get(data.id);

    if (controller) controller.update(data);
    else this._roomControllerCache.set(
      data.id,
      new RoomController(data, this._client)
    );
  }
  ...
  get top() {
    return new Promise((resolve, _reject) => {
      this._client.api.onMessageOnce(
        OP_CODE.GET_TOP_PUBLIC_ROOMS_DONE,
        (msg) => {
          resolve(msg.d.rooms);
        }
      );
  ...
  join(roomUnresolved) {
    return new Promise(async (resolve, reject) => {
        ...
        typeof roomUnresolved === "string"
          ...
          : "id" in roomUnresolved
            ...
            : "room" in roomUnresolved && "id" in roomUnresolved.room
              ...
              : "d" in roomUnresolved &&
            "room" in roomUnresolved.d &&
            "id" in roomUnresolved.d.user
      ...
      await this._client.api.send(OP_CODE.JOIN_ROOM, { roomId }, null);
    });
  }
