...
import { OP_CODE, EVENT, CONNECTION } from "../util/constraints";
...
class BotUser {
  ...
  askToSpeak() {
    return new Promise((resolve, reject) => {
      this._client.api
        .send(OP_CODE.ASK_TO_SPEAK, {}, null)
        .then(() => {
          const timeout = setTimeout(() => {
            throw new Error("Request to speak timed out after 60 seconds.");
          }, 60000);

          this._client.on(EVENT.SOCKET_MESSAGE, (msg) => {
            clearTimeout(timeout);
            if (msg.op == OP_CODE.HAND_RAISED) return resolve(null);
          });
        })
        .catch((err) => {
          return reject(err);
