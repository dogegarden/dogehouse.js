...
class BotUser {
    ...
    sendMessage(message, options = {}) {
        return new Promise((resolve, reject) => {
            const tokens = format(message); // get properly formatted array of tokens
            message = tokens.map(a=>a.v).join(' ').replace(/[\x00-\x20]+/g, ' ').trim(); // get plain text of message
            ...
            this._client.api.send(OP_CODE.SEND_ROOM_CHAT_MSG, reqOpts, null).catch(reject)

            const text = messageToString(tokens, { plain: false });
            this._client.registerIncommingChatMessageListener(text, resolve);
        });
    }
