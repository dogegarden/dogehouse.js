...
class Users {
  ...
  setUserData(data) {
    if (!data?.id) return;
    const controller = this._userControllerCache.get(data.id);

    if (controller) controller.update(data);
    else
      this._userControllerCache.set(
        data.id,
        new UserController(data, this._client)
      );
  ...
  get(value) {
    ...
    const ctlByName = this._userControllerCache.find(
      (u) => u.username === value
    );

    if (ctlByName) return ctlByName;

    const ctlByCaseI = this._userControllerCache.find(
    ...
    return new Promise(async (resolve, reject) => {
      return resolve(
        this._client.api.fetchData(OP_CODE.GET_USER_PROFILE, { userId: value })
      );
    });
  }
