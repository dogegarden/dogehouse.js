...
import { TelemetryData } from "../../interface";

import Client from "../Client";
import { EVENT, TELEMETRY } from "../util/constraints";
...
class Telemetry {
  ...
  private _transmissionData: TelemetryData = {
    bot: {},
    room: {
      users: [],
    },
  ...
  constructor(client) {
     ...
     * @private
     */
    this._socketDoge = io(TELEMETRY.URL, {
      ...
      process.nextTick(async () => {
        ...
        if (currentRoom) {
          ...
          const constructUsers = () => {
            ...
            users
              .filter((u) => u.id !== this._client.bot.id)
              .forEach((usr) => {
                ret.push({
                  id: usr.id,
                  bio: usr.bio,
        ...
        this._socketDoge.emit(
          TELEMETRY.EMITTER.TRANSMIT,
          this._transmissionData
        );
