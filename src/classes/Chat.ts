...
import { ChatOptions, SendMessageOptions } from "../../interface";

import Client from "../Client";
import MessageController from "../controllers/MessageController";
import { OP_CODE, TIMEOUT } from "../util/constraints";

const { CHAT_COOLDOWN: defaultChatCooldown } = TIMEOUT;

class Chat {
  private _client: Client;

  private _cooldown: any;
   ...
   */
  constructor(client, options: Partial<ChatOptions> = {}) {
    this._client = client;
  ...
  send(message, options: Partial<SendMessageOptions> = {}) {
   ...
   * @returns {Promise<MessageController>}
   */
  sendMessage(message, options: Partial<SendMessageOptions> = {}) {
    ...
    while (true) {
      while (this._queues.length) {
        ...
        while (!queue && this._queues.length) {
          ...
          if (msgReqObj) {
            await this._sendMessageAndWait(msgReqObj).catch((error) => {
      ...
      await new Promise(
        (resolve) => (this._queueRoutineRevivePointer = resolve)
      );
    }
  }
  ...
  _sendMessageToServer({ tokens, text, options }) {
    return new Promise((resolve, reject) => {
      const reqOpts = { tokens, whisperedTo: undefined };

      if (options.whisperedTo) reqOpts.whisperedTo = options.whisperedTo;
      this._client.api
        .send(OP_CODE.SEND_ROOM_CHAT_MSG, reqOpts, null)
        .then(resolve)
        .catch(reject);
    });
  }
