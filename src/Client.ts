...
import { CONNECTION, EVENT, OP_CODE, TELEMETRY } from "./util/constraints";
...
class Client extends BaseClient {
  ...
  constructor(options: Partial<ClientOptions> = {}) {
    ...
    /** @private */
    this._sendTelemetry =
      options.sendTelemetry !== undefined ? options.sendTelemetry : true;

    /** @private */
    this._blockAngular =
      options.blockAngular !== undefined ? options.sendTelemetry : false;
  ...
  async connect(token, refreshToken) {
    return new Promise(async (resolve, reject) => {
      const options = {
        connectionTimeout: CONNECTION.CONNECTION_TIMEOUT,
        WebSocket: isoWS,
      };
      const socket = new ReconnectingWebSocket(CONNECTION.API_URL, [], options);

      if (!token)
        throw new Error("Token must be defined to connect to Dogehouse.");
      if (!refreshToken)
        throw new Error(
          "Refresh token must be defined to connect to Dogehouse"
        );
      ...
      socket.addEventListener("open", () => {
        ...
        socket.addEventListener("close", (err) => {
          ...
          if (err.code == 4003)
            return reject(new Error("Unable to authenticate connection."));
          if (err.code == 4001)
            return reject(new Error("Socket connection taken."));
          if (err.code == 1011)
            return reject(new Error("Invalid or missing tokens"));
      ...
      await this.registerEvents(path.resolve(__dirname, "events")).then(
        async () => {
          await this._eventCache.forEach((fn) => {
            if (fn instanceof Function) fn(this);
          });
