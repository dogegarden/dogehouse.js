...
import { MessageData, MessageReplyOptions, MessageToken } from "../../interface";
...
class MessageController {
  ...
  private _rawData: Partial<MessageData>;
   ...
   */
  constructor(data: Partial<MessageData> = {}, client: Client) {
    this._client = client;
  ...
  get mentions() {
    ...
    this.tokens.forEach((tk) => {
      if (tk.t === "mention")
        mentions.push({
          user: this._client.users.cache
            .filter((u) => u.username === tk.v)
            .array()[0],
          token: tk,
        });
    });
   ...
   */
  reply(msg, options: Partial<MessageReplyOptions> = {}) {
    return new Promise(async (resolve, reject) => {
      const constructMessage = () => {
        const m = [];

        if (options.mentionUser || options.mentionUser === undefined)
          m.push({ mention: this._author.username });
        m.push(msg);

        return m;
      };

      if (!options.whispered)
        await this._client.bot.sendMessage(constructMessage()).then(resolve);
      else
        this._client.bot.sendMessage(constructMessage(), {
          whisperedTo: [this.author.id],
        });
    });
  }
  ...
  delete() {
    return new Promise(async (resolve, reject) => {
      await this._client.api
        .send(
          OP_CODE.DELETE_ROOM_CHAT_MESSAGE,
          { messageId: this.id, userId: this.author.id },
          null
        )
        .then(() => {
          return resolve(this);
