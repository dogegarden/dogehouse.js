import { RoomUserTelemetryData } from "../../interface";
...
class UserController {
   ...
   */
  constructor(data: Partial<User> = {}, client: Client) {
  ...
  follow() {
    return new Promise((resolve, reject) => {
      this._client.api
        .send(OP_CODE.FOLLOW, { userId: this.id, value: true }, null)
        .then(() => {
          resolve(this);
        })
        .catch(reject);
  ...
  unfollow() {
    return new Promise((resolve, reject) => {
      this._client.api
        .send(OP_CODE.FOLLOW, { userId: this.id, value: false }, null)
        .then(() => {
          resolve(this);
        })
        .catch(reject);
  ...
  whisper(msg) {
    return new Promise((resolve, reject) => {
      this._client.bot
        .sendMessage(msg, { whisperedTo: [this.id] })
        .then((msg) => {
          resolve(msg);
        })
        .catch(reject);
  ...
  mention(msg) {
    return new Promise((resolve, reject) => {
      this._client.bot
        .sendMessage([{ mention: this.username }, msg])
        .then((msg) => {
          resolve(msg);
        })
        .catch(reject);
  ...
  setListener() {
    return new Promise((resolve, reject) => {
      this._client.api
        .send(OP_CODE.SET_LISTENER, { userId: this.id }, null)
        .then(() => {
          resolve(this);
        })
        .catch(reject);
  ...
  setSpearker() {
    return new Promise((resolve, reject) => {
      this._client.api
        .send(OP_CODE.ADD_SPEAKER, { userId: this.id }, null)
        .then(() => {
          resolve(this);
        })
        .catch(reject);
...
interface User extends RoomUserTelemetryData {
  lastOnline: string;
  followsYou: boolean;
  avatarUrl: string;
}
